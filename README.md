...
- **Видит, где режут.** Знать, что цель не отвечает, мало. Klutz стучится в
  ...
  или отказывает сам сервер (это вообще не блокировка). Самолечение это
  учитывает и не гоняет стратегии там, где ни одна не спасёт.
...
- **Дополнительные стратегии.** Когда ни один штатный конфиг не пробивает,
  одна кнопка кладёт в папку релиза восемь вариантов текущего с другими
  точками разреза — и убирает их так же. Дальше они живут как обычные
  конфиги: попадают в прогон, в рейтинг и в самолечение.
...
имени», «Блок по адресу», «Отказ сервера», «Блок по закону» или честное «Не
измерено»; на чём вердикт основан — в подсказке строки.
...
оттуда же взяты точки разреза для дополнительных стратегий. Движки у нас
