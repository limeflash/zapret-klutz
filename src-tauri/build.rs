fn main() {
    // Zapret needs to spawn winws.exe (WinDivert packet filtering), which
    // requires admin rights — the Electron build set this via
    // requestedExecutionLevel in package.json; here it's an embedded Windows
    // manifest, same as the .exe would need either way.
