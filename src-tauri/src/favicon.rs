//! Иконки сервисов.
//!
//! Качаем сами и только с сайта самого сервиса. Через чужой «сервис иконок»
//! было бы надёжнее — он знает и про служебные поддомены, — но тогда список
//! того, что человек ищет в программе для обхода блокировок, уходил бы
//! третьей стороне одним запросом на каждую строку поиска. Здесь этого не
//! происходит: запрос идёт ровно туда же, куда и так ходит пользователь, и
//! через тот же обход.
//!
...
/// Сколько ждать. Иконка — украшение, ради неё нельзя подвешивать поиск.
const TIMEOUT: &str = "8";
...
/// после того, как обход заработал.
const NEGATIVE_TTL: Duration = Duration::from_secs(24 * 60 * 60);

/// Публичные суффиксы из двух меток, которые реально встречаются у игровых
/// и медийных сервисов. Полный Public Suffix List тянуть ради иконок не
...
const TWO_LABEL_TLDS: &[&str] = &[
    ...
    "com.mx", "com.ar", "com.pl", "co.nz", "co.za", "com.ua", "com.sg", "com.hk",
];

/// Домен бренда. В каталоге хосты служебные — `api.rlpp.psynet.gg`,
/// `title.mgt.xboxlive.com`, — а иконка лежит на основном домене.
