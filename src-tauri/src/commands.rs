...
pub fn scan_game_from_log(
    ...
    let note = if addrs.is_empty() {
        ...
        format!(
            "поймано серверов: {}. Взяты из пакетов, которые видел сам обход, — сюда \
             попадает и игровой UDP, невидимый в таблице соединений. В список кладём \
             не сам адрес, а его сеть /24: матч подключается к одному серверу из пула, \
             и следующий будет соседним",
            addrs.len()
        )
    };
